...
if not ((output_file=='contigs.fasta') or (output_file=='scaffolds.fasta')):
    raise Exception("First output file should be folder/contigs.fasta or folder/scaffolds.fasta")
...
# parse params
    ...
    assert len(reads)>1, "Metaspades needs a paired end library. This means you should supply at least 2 fastq files in the rule input."
    ...
    for longread_name in ['pacbio','nanopore']:
        if hasattr(snakemake.input,longread_name):
            input+=f" --{name} {}".format(name=longread_name,**snakemake.input)
