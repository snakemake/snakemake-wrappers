...
pep_query=snakemake.input.get("pep")
nucl_query=snakemake.input.get("nucl")
...
assert (list(map(bool, queryfiles)).count(True) == 1), "one query is required: either a peptide query (pep_query) for diamond blastp OR a nucleotide query (nucl_query) for diamond blastx"
...
if sensitivity:
    if sensitivity == "sensitive":
       ...
       # more-sensitive: This mode provides some additional sensitivity compared to the sensitive mode.
        ...
        raise ValueError("alignment sensitivity must be either 'sensitive' or 'more-sensitive'")

shell("diamond {prog} -db {snakemake.input.db} -q {query} -o {output} --threads {snakemake.threads} {txlist_cmd} {s_cmd} {extra} {log}")
