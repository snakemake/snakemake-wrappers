"""Snakemake wrapper for Transdecoder Predict"""

__author__ = "N. Tessa Pierce"
__copyright__ = "Copyright 2019, N. Tessa Pierce"
__email__ = "ntpierce@gmail.com"
__license__ = "MIT"

from pathlib import Path
from snakemake.shell import shell

extra = snakemake.params.get("extra", "")
log = snakemake.log_fmt_shell(stdout=True, stderr=True)

input_fasta = str(snakemake.input.fasta)
if input_fasta.endswith(".gz"):
    input_fa = Path(input_fasta).with_suffix("")
    shell("gunzip -c {input_fasta} > {input_fa}")
else:
    input_fa = input_fasta

pfam = snakemake.input.get("pfam_hits", "")
if pfam:
    pfam = f"--retain_pfam_hits {pfam}"

blastp = snakemake.input.get("blastp_hits", "")
if blastp:
    blastp = f"--retain_blastp_hits {blastp}"

output_dir = Path(snakemake.output[0]).parent

shell(
    "TransDecoder.Predict -t {input_fa} {pfam} {blastp} {extra} --output_dir {output_dir} {log}"
)
